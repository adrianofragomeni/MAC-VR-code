...
class RetrievalDataset(Dataset):
    ...
    def __init__(
        ...
        self.video_dict, self.sentences_dict,self.textual_concept_dict, self.visual_concept_dict = self._get_anns(self.subset)
        #print(self.video_dict, self.sentences_dict, self.textual_concept_dict, self.visual_concept_dict)
    ...
    def _get_text_concept(self, concept):
        if len(concept) == 3:
            _concept_text, s, e = concept
        else:
            raise NotImplementedError

        if isinstance(_concept_text, list):
            concept_text = random.choice(_concept_text)
        else:
            concept_text = _concept_text

        words = self.tokenizer.tokenize(concept_text)
    ...
    def _get_rawvideo_dec(self, video_id, s=None, e=None):
        ...
        if exists(video_path):
            gif =   Image.open(video_path)    #VideoReader(video_path, ctx=cpu(0))
            try:
                vreader= [frame for frame in ImageSequence.Iterator(gif.convert("RGB"))]
            except:
                print(video_path)
        else:
            print(video_path)
        ...
        if num_frames > 0:
            ...
            t_stride = 1 #int(round(float(fps) / sample_fps))
            ...
            patch_images = [Image.fromarray(np.array(vreader[pos])).convert("RGB") for pos in range(len(vreader)) if pos in sample_pos]
            if self.subset == "train":
                # for i in range(2):
                #print(video_path)
                #print(np.array(patch_images[0]).shape)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'all':
            ...
            _, visual_concept = self.visual_concept_dict[idx]
            _, textual_concept = self.textual_concept_dict[idx]

            text_ids, text_mask, s, e = self._get_text(caption)
            visual_concepts_list=ast.literal_eval(visual_concept[0])
            textual_concepts_list=ast.literal_eval(textual_concept[0])

            if self.subset == "train":
                visual_concept[0]="a video of "+", ".join(random.sample(visual_concepts_list,min(self.config.number_visual_tags_train,len(visual_concepts_list))))
                visual_concept=tuple(visual_concept)

               	textual_concept[0]="a video of "+", ".join(random.sample(textual_concepts_list,min(self.config.number_textual_tags_train,len(textual_concepts_list))))
                textual_concept=tuple(textual_concept)
            else:
                visual_concept[0]="a video of "+", ".join(visual_concepts_list[:self.config.number_visual_tags_test])
                visual_concept=tuple(visual_concept)

                textual_concept[0]="a video of "+", ".join(textual_concepts_list[:self.config.number_textual_tags_test])
                textual_concept=tuple(textual_concept)

            visual_c_ids, visual_c_mask, visual_c_s, visual_c_e = self._get_text_concept(visual_concept)
            textual_c_ids, textual_c_mask, textual_c_s, textual_c_e = self._get_text_concept(textual_concept)


            video, video_mask = self._get_rawvideo_dec(video_id, s,e)
            # video, video_mask = self._get_rawvideo(video_id, s, e)
            return text_ids, text_mask, visual_c_ids, visual_c_mask, textual_c_ids, textual_c_mask, video, video_mask, idx, hash(str(video_id).replace("video", ""))
        elif self.mode == 'text':
            video_id, caption = self.sentences_dict[idx]
            text_ids, text_mask, s, e = self._get_text(caption)
