...
class DiCoSA(nn.Module):
    def __init__(self, config):
        ...
        self.weight_fc = nn.Sequential(
                    nn.Linear(4*width, 8*width), nn.ReLU(inplace=True),
                    nn.Linear(8*width, 1))
    ...
    def forward(self, text_ids, text_mask,visual_c_ids, visual_c_mask, textual_c_ids, textual_c_mask, video, video_mask=None, idx=None, global_step=0):
        ...
        text_feat, video_feat, cls, cls_visual_c,cls_textual_c = self.get_text_video_feat(text_ids, text_mask, video, video_mask, visual_c_ids, visual_c_mask,textual_c_ids, textual_c_mask,shaped=True)
        if self.training:
            if torch.cuda.is_available():  # batch merge here
                ...
                visual_c_mask = allgather(visual_c_mask, self.config)
                textual_c_mask = allgather(textual_c_mask, self.config)
                text_mask = allgather(text_mask, self.config)
                video_mask = allgather(video_mask, self.config)
                cls_visual_c = allgather(cls_visual_c, self.config)
                cls_textual_c = allgather(cls_textual_c, self.config)
            ...
            M_t2v_logits, z_v_norm, z_t_norm, z_a_norm, z_b_norm, M_v2t_logits, inter_loss, intra_t_loss, intra_v_loss = self.get_similarity_logits(text_feat, cls, cls_visual_c, cls_textual_c, video_feat,
            ...
            loss = M_loss + inter_loss + intra_t_loss + intra_v_loss
            return loss, M_loss, inter_loss, intra_t_loss, intra_v_loss
        else:
            return None

    def similarity(self, text_feat, cls,  cls_visual_c, cls_textual_c, video_feat, text_mask, video_mask):
        ...
        cls, video_feat,cls_visual_c, cls_textual_c = cls.contiguous(), video_feat.contiguous(),cls_visual_c.contiguous(), cls_textual_c.contiguous()
        
        visual_c_feat = cls_visual_c.view(b, self.config.center, -1)
        textual_c_feat = cls_textual_c.view(a, self.config.center, -1)
        
        t_feat = cls.view(a, self.config.center, -1)
        v_feat = video_feat.view(a, b, self.config.center, -1)
        d = t_feat.size(2)
        #adaptive pooling
        temp = torch.cat([t_feat.unsqueeze(1).repeat(1, b, 1, 1), visual_c_feat.unsqueeze(0).repeat(a, 1, 1, 1), textual_c_feat.unsqueeze(1).repeat(1, b, 1, 1), v_feat], dim=-1) # a b c 2d

        weight = self.weight_fc(temp).squeeze(3)  # a b c 2d-> a b c

        _visual_c_feat = visual_c_feat / visual_c_feat.norm(dim=-1, keepdim=True)
        _textual_c_feat = textual_c_feat / textual_c_feat.norm(dim=-1, keepdim=True)
        ...
        if self.training:
            _visual_c = _visual_c_feat.permute(0,2,1).contiguous()
            _textual_c = _textual_c_feat.permute(0,2,1).contiguous()
            
            _t = t_feat.permute(0,2,1).contiguous()
            _v = torch.diagonal(v_feat, dim1=0, dim2=1).permute(2,1,0).contiguous()
            
            z_v_norm = (_visual_c - _visual_c.mean(1).unsqueeze(1)) / _visual_c.std(1).unsqueeze(1)
            z_t_norm = (_textual_c - _textual_c.mean(1).unsqueeze(1)) / _textual_c.std(1).unsqueeze(1)
            ...
            on_diag_2 = torch.diagonal(c).add_(-1).pow_(2).sum()
            off_diag_2 = c.flatten()[1:].view(D - 1, D + 1)[:, :-1].pow_(2).sum()
            inter_loss = (on_diag_2 * self.config.alpha + off_diag_2 * self.config.beta)
            
            # loss IntraT
            c_c = torch.einsum("adb,adc->abc", [z_a_norm, z_t_norm])   # BxCxC
            c_c = c_c.mean(0) / text_feat.shape[1]                       # CxC     
            on_diag_1 = torch.diagonal(c_c).add_(-1).pow_(2).sum()
            off_diag_1 = c_c.flatten()[1:].view(D - 1, D + 1)[:, :-1].pow_(2).sum()
            intra_t_loss = (on_diag_1 * self.config.alpha + off_diag_1 * self.config.beta)       
            
            # loss IntraV
            c_v = torch.einsum("adb,adc->abc", [z_v_norm, z_b_norm])   # BxCxC
            c_v = c_v.mean(0) / text_feat.shape[1]                       # CxC
            on_diag_3 = torch.diagonal(c_v).add_(-1).pow_(2).sum()
            off_diag_3 = c_v.flatten()[1:].view(D - 1, D + 1)[:, :-1].pow_(2).sum()
            intra_v_loss = (on_diag_3 * self.config.alpha + off_diag_3 * self.config.beta)
        ...
        return retrieve_logits, retrieve_logits.T, inter_loss, intra_t_loss, intra_v_loss, _visual_c_feat, _textual_c_feat, _t_feat, _v_feat
    ...
    def get_text_video_feat(self, text_ids, text_mask, video, video_mask, visual_c_ids, visual_c_mask,textual_c_ids, textual_c_mask, shaped=False):
        if shaped is False:
            text_ids = text_ids.view(-1, text_ids.shape[-1])
            text_mask = text_mask.view(-1, text_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
            ...
            visual_c_ids = visual_c_ids.view(-1, visual_c_ids.shape[-1])
            visual_c_mask = visual_c_mask.view(-1, visual_c_mask.shape[-1])
            textual_c_ids = textual_c_ids.view(-1, textual_c_ids.shape[-1])
            textual_c_mask = textual_c_mask.view(-1, textual_c_mask.shape[-1])
        ...
        visual_c_feat, visual_cls = self.get_text_feat(visual_c_ids, visual_c_mask, shaped=True)
        tedxtual_c_feat, textual_cls = self.get_text_feat(textual_c_ids, textual_c_mask, shaped=True)
        video_feat = self.get_video_feat(video, video_mask, shaped=True)

        return text_feat, video_feat, cls, visual_cls, textual_cls
    ...
    def get_similarity_logits(self, text_feat, cls,cls_visual_c, cls_textual_c, video_feat, text_mask, video_mask, shaped=False):
        if shaped is False:
            text_mask = text_mask.view(-1, text_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])

        M_t2v_logits, M_v2t_logits, logits, logits_t, logits_v,  z_v_norm, z_t_norm, z_a_norm, z_b_norm  = self.similarity(text_feat, cls,cls_visual_c, cls_textual_c, video_feat, text_mask, video_mask)
        
        return M_t2v_logits, z_v_norm, z_t_norm, z_a_norm, z_b_norm, M_v2t_logits, logits, logits_t, logits_v
