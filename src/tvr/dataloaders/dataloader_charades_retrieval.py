...
class CharadesDataset(RetrievalDataset):
    ...
    def _get_anns(self, subset='train'):
        ...
        """
        csv_path = {'train': join(self.anno_path, 'Charades_v1_train.csv'),
                    'val': join(self.anno_path, 'Charades_v1_test.csv'),
                    'test': join(self.anno_path, 'Charades_v1_test.csv'),
                    'train_test': join(self.anno_path, 'Charades_v1_train.csv')}[subset]
        if exists(csv_path):
        ...
        if subset == 'train':
            for row in csv.values:
                if row[0] in video_id_list:
                    ...
                    video_dict[row[0]] = join(self.video_path, "{}.mp4".format(row[0]))
