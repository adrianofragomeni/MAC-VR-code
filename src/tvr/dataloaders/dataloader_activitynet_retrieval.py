...
class ActivityNetDataset(Dataset):
    def __init__(
        ...
        self.subset = subset
        assert self.subset in ["train", "val", "train_test"]

        self.config = config
        video_id_path_dict = {}
        video_id_path_dict["train"] = os.path.join(self.data_path, "train_ids.json")
        video_id_path_dict["train_test"] = os.path.join(self.data_path, "train_ids.json")
        ...
        video_json_path_dict["train_test"] = os.path.join(self.data_path, "train.json")
    ...
    def _get_text_concept(self, concept_text):
    ...
    def __getitem__(self, feature_idx):
        ...
        visual_concepts_list=ast.literal_eval(self.concepts[pseudo_video_id][0])
        textual_concepts_list=ast.literal_eval(self.concepts[pseudo_video_id][1])
        
        if self.subset == "train":
            visual_concept="a video of "+", ".join(random.sample(visual_concepts_list,min(self.config.number_visual_tags_train,len(visual_concepts_list))))
            textual_concept="a video of "+", ".join(random.sample(textual_concepts_list,min(self.config.number_textual_tags_train,len(textual_concepts_list))))
            
        else:
            visual_concept="a video of "+", ".join(visual_concepts_list[:self.config.number_visual_tags_test])
            textual_concept="a video of "+", ".join(textual_concepts_list[:self.config.number_textual_tags_test])
            
        visual_c_ids, visual_c_mask= self._get_text_concept(visual_concept)
        textual_c_ids, textual_c_mask = self._get_text_concept(textual_concept)
        ...
        return pairs_text, pairs_mask, visual_c_ids, visual_c_mask, textual_c_ids, textual_c_mask, video, video_mask, feature_idx, hash(pseudo_video_id)
